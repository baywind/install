...
CREATE DATABASE RujelYear2013 DEFAULT CHARACTER SET utf8;
USE RujelYear2013;
...
CREATE INDEX ItogRelatedCourseIdx
...
CREATE INDEX PrognosisFireDateIdx
  ON AI_PROGNOSIS (FIRE_DATE);

CREATE INDEX PrognosisStudentIdx
  ON AI_PROGNOSIS (STUDENT_ID);
...
CREATE INDEX StudentTimeoutCourseIdx
  ON AI_STUDENT_TIMEOUT (EDU_COURSE);

CREATE INDEX StudentTimeoutItogIdx
...
CREATE INDEX AudienceStudentIdx
  ON BASE_AUDIENCE (STUDENT_ID);
...
CREATE INDEX LessonCourseIdx
  ON BASE_LESSON (EDU_COURSE);

CREATE INDEX LessonDateIdx
  ON BASE_LESSON (DATE_PERFORMED);
...
CREATE INDEX CompletionCourseIdx
  ON CPT_COMPLETION (EDU_COURSE);

CREATE INDEX CompletionStudentIdx
...
CREATE INDEX DecisionStudentIdx
...
CREATE INDEX NoteStudentIdx
  ON CR_NOTE (STUDENT_ID);
...
CREATE INDEX ReprimandCourseIdx
...
CREATE INDEX VariationCourseIdx
  ON CU_VARIATION (EDU_COURSE);

CREATE INDEX VariationLessonIdx
...
CREATE INDEX MarkArchiveObjIdx
  ON MA_MARK_ARCHIVE (OBJ_ENTITY);

CREATE INDEX MarkArchiveKeysIdx
  ON MA_MARK_ARCHIVE (KEY2_VALUE, KEY1_VALUE, KEY3_VALUE);

CREATE INDEX MarkArchiveTimeIdx
  ON MA_MARK_ARCHIVE (CHANGE_TIME);

CREATE INDEX MarkArchiveTypeIdx
  ON MA_MARK_ARCHIVE (ACTION_TYPE);
...
CREATE INDEX SchedulrCourseIdx
...
CREATE INDEX StatEntryGroupingIdx
...
CREATE INDEX ItogCommentStudentIdx
  ON ITOG_COMMENT (STUDENT_ID);
...
CREATE INDEX ItogMarkStudentIdx
  ON ITOG_MARK (STUDENT_ID);
...
CREATE INDEX VseListGroupIdx
  ON VSE_LIST (EDU_GROUP);
...
CREATE INDEX ContactPersonIdx
  ON CNT_CONTACT (PERSON_ID);
...
CREATE INDEX SyncMatchBaseIdx
  ON SYNC_MATCH (SYSTEM_ID, BASE_ID);

CREATE INDEX SyncMatchObjectIdx
...
CREATE INDEX SyncEventIdx
  ON SYNC_EVENT (SYSTEM_ID, BASE_ID);
