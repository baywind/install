...
SELECT * from RujelStatic.BASE_COURSE where EDU_YEAR = '2010';
...
UPDATE BASE_INDEXER
  ...
  WHERE COMMENT_ID IS NOT NULL AND @textmax IS NOT NULL;
...
INSERT INTO BASE_TEXT_STORE (T_ID, FROM_ENTITY, STORED_TEXT)
  SELECT if(@textmax IS NULL, T_ID, T_ID + @textmax) AS ID, FROM_ENTITY, STORED_TEXT
  FROM RujelStatic.BASE_ST_TEXT_STORE;
...
WHERE EDU_YEAR IS NULL OR EDU_YEAR = '2010';
...
ALTER TABLE AI_COURSE_TIMEOUT
  MODIFY COLUMN EDU_COURSE smallint,
  MODIFY COLUMN EDU_GROUP int,
  MODIFY COLUMN EDU_CYCLE smallint,
  MODIFY COLUMN TEACHER_ID mediumint;

ALTER TABLE AI_ITOG_RELATED
  MODIFY COLUMN EDU_COURSE smallint NOT NULL;

ALTER TABLE AI_PROGNOSIS
  MODIFY COLUMN EDU_COURSE smallint NOT NULL;

ALTER TABLE AI_STUDENT_TIMEOUT
  MODIFY COLUMN EDU_COURSE smallint;
...
FROM RujelStatic.PL_CYCLE where EDU_YEAR = 0 OR EDU_YEAR >= 10;