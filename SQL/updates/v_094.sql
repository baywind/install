CREATE DATABASE `RujelSync` DEFAULT CHARACTER SET utf8;
USE `RujelSync`;

CREATE TABLE IF NOT EXISTS SYNC_EXT_SYSTEM (
  S_ID smallint NOT NULL,
  PRODUCT_NAME varchar(255),
  SYSTEM_DESCRIPTION text,
  PRIMARY KEY (S_ID)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS SYNC_EXT_BASE (
  B_ID smallint NOT NULL,
  SYSTEM_ID smallint NOT NULL,
  BASE_IDENTIFIER varchar(255) NOT NULL,
  BASE_TITLE varchar(255),
  PRIMARY KEY (B_ID)
) ENGINE=InnoDB;


CREATE TABLE IF NOT EXISTS SYNC_EXT_DATA (
  D_ID integer NOT NULL,
  SYSTEM_ID smallint NOT NULL,
  BASE_ID smallint,
  DATA_KEY varchar(28) NOT NULL,
  DATA_VALUE text,
  EDU_YEAR smallint,
  FOR_SECTION smallint,
  PRIMARY KEY (D_ID),
  INDEX (SYSTEM_ID, BASE_ID)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS SYNC_MATCH (
  M_ID integer NOT NULL,
  SYSTEM_ID smallint NOT NULL,
  BASE_ID smallint,
  ENTITY_ID smallint NOT NULL,
  OBJECT_ID integer NOT NULL,
  EXTERNAL_ID varchar(255),
  EDU_YEAR smallint,
  PRIMARY KEY (M_ID),
  INDEX (SYSTEM_ID, BASE_ID),
  INDEX (ENTITY_ID, OBJECT_ID)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS SYNC_INDEX (
  I_ID integer NOT NULL,
  SYSTEM_ID smallint NOT NULL,
  BASE_ID smallint,
  INDEX_NAME varchar(28) NOT NULL,
  EDU_YEAR smallint,
  PRIMARY KEY (I_ID),
  INDEX (SYSTEM_ID, BASE_ID)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS SYNC_IDX_MATCH (
  M_ID integer NOT NULL,
  INDEX_ID integer NOT NULL,
  LOCAL_VALUE varchar(255),
  EXT_VALUE varchar(255),
  PRIMARY KEY (M_ID),
  INDEX (INDEX_ID)
) ENGINE=InnoDB;


CREATE TABLE IF NOT EXISTS EO_PK_TABLE (
  NAME char(40),
  PK int
);

CREATE TABLE IF NOT EXISTS SCHEMA_VERSION (
  MODEL_NAME varchar(255),
  VERSION_NUMBER smallint unsigned NOT NULL,
  VERSION_TITLE varchar(255),
  INSTALL_DATE timestamp
);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('RujelSync',1,'0.9.4');


USE `RujelYear2011`;

ALTER TABLE BASE_COURSE ADD INDEX (EDU_GROUP);
ALTER TABLE BASE_LESSON ADD INDEX (EDU_COURSE);
ALTER TABLE BASE_TAB ADD INDEX (EDU_COURSE);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('BaseYearly',2,'0.9.4');


ALTER TABLE CR_WORK ADD INDEX (EDU_COURSE);
ALTER TABLE CR_MARK ADD INDEX (STUDENT_ID);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('Criterial',1,'0.9.4');

ALTER TABLE SDL_SCHEDULE ADD INDEX (EDU_COURSE);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('Schedule',2,'0.9.4');


ALTER TABLE AI_ITOG_RELATED ADD INDEX (EDU_COURSE);
ALTER TABLE AI_STUDENT_TIMEOUT ADD INDEX (EDU_COURSE);
ALTER TABLE AI_STUDENT_TIMEOUT ADD INDEX (ITOG_CONTAINER);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('AutoItog',1,'0.9.4');


ALTER TABLE CU_REPRIMAND ADD INDEX (EDU_COURSE);
ALTER TABLE CU_VARIATION ADD INDEX (LESSON_ID);
ALTER TABLE CU_VARIATION ADD INDEX (EDU_COURSE);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('Curriculum',2,'0.9.4');


CREATE INDEX ind1 ON MA_MARK_ARCHIVE(KEY2_VALUE, KEY1_VALUE);
ALTER TABLE MA_MARK_ARCHIVE ADD INDEX (OBJ_ENTITY);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('MarkArchive',2,'0.9.4');


ALTER TABLE PL_HOURS ADD INDEX (CYCLE_ID);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('EduPlanYearly',2,'0.9.4');


ALTER TABLE ST_ENTRY ADD INDEX (GROUPING_ID);
ALTER TABLE ST_GROUPING ADD INDEX (GR_ID1);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('Stats',1,'0.9.4');

ALTER TABLE CPT_COMPLETION ADD INDEX (EDU_COURSE);
ALTER TABLE CPT_COMPLETION ADD INDEX (STUDENT_ID);

CREATE TABLE IF NOT EXISTS CPT_PED_DECISION (
  P_ID smallint NOT NULL,
  STUDENT_ID int NOT NULL,
  SPEC_DECISION varchar(255),
  DECISION_FLAGS tinyint NOT NULL,
  PRIMARY KEY (P_ID),
  UNIQUE INDEX (STUDENT_ID)
) ENGINE=InnoDB;

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('Complete',1,'0.9.4');

USE `VseLists`;

ALTER TABLE VSE_LIST ADD INDEX (EDU_GROUP);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('VseLists',2,'0.9.4');

USE `RujelStatic`;

ALTER TABLE ITOG_COMMENT ADD INDEX (STUDENT_ID);
ALTER TABLE ITOG_MARK ADD INDEX (STUDENT_ID);

CREATE TABLE IF NOT EXISTS SCHEMA_VERSION (
  MODEL_NAME varchar(255),
  VERSION_NUMBER smallint unsigned NOT NULL,
  VERSION_TITLE varchar(255),
  INSTALL_DATE timestamp
);

INSERT INTO SCHEMA_VERSION (MODEL_NAME,VERSION_NUMBER,VERSION_TITLE)
  VALUES ('EduResults',1,'0.9.4');
